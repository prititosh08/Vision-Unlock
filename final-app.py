...
def draw_boundary(img, classifier, scaleFactor, minNeighbors, color, clf):
    ...
    if current_status != last_status and time.time() - last_sent_time > cooldown:
        ...
        if current_status == "legend" or current_status == "Manish":
            ...
            try:
                response = requests.get("http://127.0.0.1:5000/buzzer")  # or whatever your host:port is
                print("Buzzer API called:", response.status_code)
            except Exception as e:
                print("Error calling /buzzer:", e)
